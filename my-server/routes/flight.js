...
router.post("/", async (req, res) => {
    ...
    const ranfactor = Math.floor(Math.random() * 100) + 1;    
    if (ranfactor <= 50) {
        try {
            //使用.save()將資料存進資料庫
...
async function getflight(req,res,next) {
    let flight;
    try {
        flight = await Flight.findOne({ _id: req.params.id }).exec();

        if (flight == undefined) {
            return res.status(404).json({ message:"Can't find flight" })
        }
    } catch (err) {
